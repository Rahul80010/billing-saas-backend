...
describe('Products API', () => {
  ...
  it('should create a new product with unit kg and buyingCost', async () => {
    const res = await request(app)
      .post('/api/products')
      .set('Authorization', `Bearer ${token}`)
      .send({
    ...
    expect(res.body.buyingCost).toBe(25);
...
describe('Bills API', () => {
  ...
  it('should create a bill with decimal quantities, units, and buyingCost, and decrement product stock', async () => {
    const Product = mongoose.model('Product');
    
    // 1. Create a product with unit kg, buyingCost and stock 10
    ...
    // 2. Create a bill with quantity 1.5 kg and buyingCost 50
    const billRes = await request(app)
      ...
      .send({
        ...
        items: [
          { productName: 'Basmati Rice', price: 80, quantity: 1.5, gst: 5, unit: 'kg', buyingCost: 50 } // (80*1.5) + 5% = 120 + 6 = 126
        ]
      });
    ...
    expect(billRes.body.items[0].buyingCost).toBe(50);
